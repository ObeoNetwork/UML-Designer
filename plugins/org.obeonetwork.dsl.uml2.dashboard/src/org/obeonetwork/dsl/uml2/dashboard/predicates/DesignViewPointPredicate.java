...
/**
 * Check if UML designer Design viewpoint is enable.
 *
 * @author Frederic Bats<a href="mailto:frederic.bats@obeo.fr">frederic.bats@obeo.fr</a>
 */
public class DesignViewPointPredicate implements IPredicate {

	/**
	 * Constructor.
	 */
	public DesignViewPointPredicate() {
		super();
	}
	...
	public boolean isOk() {
		...
		if (ActivityExplorerManager.INSTANCE.getSession() != null) {
			return DashboardServices.INSTANCE.isEnabledVP(session, DashboardServices.INSTANCE.VP_DESIGN);
		}
		return false;
	}
}
