...
/**
 * Check if UML designer Capture viewpoint is enable.
 *
 * @author Frederic Bats<a href="mailto:frederic.bats@obeo.fr">frederic.bats@obeo.fr</a>
 */
public class CaptureViewPointPredicate implements IPredicate {
	// $NON-NLS-1$

	/**
	 * Constructor.
	 */
	public CaptureViewPointPredicate() {
		super();
	}
	...
	public boolean isOk() {
		...
		if (ActivityExplorerManager.INSTANCE.getSession() != null) {
			return DashboardServices.INSTANCE.isEnabledVP(session, DashboardServices.INSTANCE.VP_CAPTURE);
		}
		return false;
	}
}
