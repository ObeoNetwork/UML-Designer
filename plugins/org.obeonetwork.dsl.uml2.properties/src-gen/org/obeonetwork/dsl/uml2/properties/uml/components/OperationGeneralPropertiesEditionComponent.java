...
public class OperationGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public OperationGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject operation, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, operation.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(operation, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getOperation_IsQuery().equals(msg.getFeature())
					&& msg.getNotifier().equals(semanticObject) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.Qualifiers.query))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getFeature_IsStatic(), UMLPackage.eINSTANCE.getOperation_IsUnique(),
				UMLPackage.eINSTANCE.getClassifier_IsAbstract(), UMLPackage.eINSTANCE.getOperation_IsQuery());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				...
				|| key == UmlViewsRepository.General.Qualifiers.query
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.General.Qualifiers.query == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(
								UMLPackage.eINSTANCE.getOperation_IsQuery().getEAttributeType(),
								(String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(
							UMLPackage.eINSTANCE.getOperation_IsQuery().getEAttributeType(), newValue);
				}
