...
public class LiteralUnlimitedNaturalPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(UmlViewsRepository.General.integerValue)) {
                generalPart.setIntegerValue(EEFConverterUtil.convertToString(TypesPackage.Literals.UNLIMITED_NATURAL, literalUnlimitedNatural.getValue()));
            }

            if (isAccessible(UmlViewsRepository.General.name))
                generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, literalUnlimitedNatural.getName()));

            if (isAccessible(UmlViewsRepository.General.visibility)) {
                generalPart.initVisibility(EEFUtils.choiceOfValues(literalUnlimitedNatural, UMLPackage.eINSTANCE.getNamedElement_Visibility()), literalUnlimitedNatural.getVisibility());
            }
            // init filters
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (UmlViewsRepository.General.integerValue == event.getAffectedEditor()) {
            literalUnlimitedNatural.setValue((EEFConverterUtil.createIntFromString(TypesPackage.Literals.UNLIMITED_NATURAL, (String) event.getNewValue())));
        }
        if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
            literalUnlimitedNatural.setName((java.lang.String) EEFConverterUtil.createFromString(TypesPackage.Literals.STRING, (String) event.getNewValue()));
        }
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (UMLPackage.eINSTANCE.getLiteralUnlimitedNatural_Value().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null
                    && isAccessible(UmlViewsRepository.General.integerValue)) {
                if (msg.getNewValue() != null) {
                    generalPart.setIntegerValue(EcoreUtil.convertToString(TypesPackage.Literals.UNLIMITED_NATURAL, msg.getNewValue()));
                } else {
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(UMLPackage.eINSTANCE.getLiteralUnlimitedNatural_Value(), UMLPackage.eINSTANCE.getNamedElement_Name(),
