...
public class PrimitiveTypePropertiesEditionComponent extends ComposedPropertiesEditionComponent {
	 ...
	 * @generated
	 */
	public PrimitiveTypePropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject primitiveType, String editing_mode) {
		super(editingContext, editing_mode);
		if (primitiveType instanceof PrimitiveType) {
			PropertiesEditingProvider provider = null;
			provider = (PropertiesEditingProvider)editingContext.getAdapterFactory().adapt(primitiveType,
					PropertiesEditingProvider.class);
			primitiveTypeGeneralPropertiesEditionComponent = (PrimitiveTypeGeneralPropertiesEditionComponent)provider
					.getPropertiesEditingComponent(editingContext, editing_mode,
							PrimitiveTypeGeneralPropertiesEditionComponent.GENERAL_PART,
							PrimitiveTypeGeneralPropertiesEditionComponent.class);
			addSubComponent(primitiveTypeGeneralPropertiesEditionComponent);
			provider = (PropertiesEditingProvider)editingContext.getAdapterFactory().adapt(primitiveType,
					PropertiesEditingProvider.class);
			primitiveTypeAttributesPropertiesEditionComponent = (PrimitiveTypeAttributesPropertiesEditionComponent)provider
					.getPropertiesEditingComponent(editingContext, editing_mode,
							PrimitiveTypeAttributesPropertiesEditionComponent.ATTRIBUTES_PART,
							PrimitiveTypeAttributesPropertiesEditionComponent.class);
			addSubComponent(primitiveTypeAttributesPropertiesEditionComponent);
			provider = (PropertiesEditingProvider)editingContext.getAdapterFactory().adapt(primitiveType,
					PropertiesEditingProvider.class);
			primitiveTypeOperationsPropertiesEditionComponent = (PrimitiveTypeOperationsPropertiesEditionComponent)provider
					.getPropertiesEditingComponent(editingContext, editing_mode,
							PrimitiveTypeOperationsPropertiesEditionComponent.OPERATIONS_PART,
							PrimitiveTypeOperationsPropertiesEditionComponent.class);
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (PrimitiveTypeGeneralPropertiesEditionComponent.GENERAL_PART.equals(key)) {
			generalPart = (GeneralPropertiesEditionPart)primitiveTypeGeneralPropertiesEditionComponent
					.getPropertiesEditionPart(kind, key);
			return (IPropertiesEditionPart)generalPart;
		}
		if (PrimitiveTypeAttributesPropertiesEditionComponent.ATTRIBUTES_PART.equals(key)) {
			attributesPart = (AttributesPropertiesEditionPart)primitiveTypeAttributesPropertiesEditionComponent
					.getPropertiesEditionPart(kind, key);
		...
		if (PrimitiveTypeOperationsPropertiesEditionComponent.OPERATIONS_PART.equals(key)) {
			operationsPart = (OperationsPropertiesEditionPart)primitiveTypeOperationsPropertiesEditionComponent
					.getPropertiesEditionPart(kind, key);
