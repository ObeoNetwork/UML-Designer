...
public class ConstraintPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public ConstraintPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject constraint,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, constraint.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(constraint,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						constraint.getVisibility());
			}
			if (isAccessible(UmlViewsRepository.General.specification)) {
				// init part
				specificationSettings = new EObjectFlatComboSettings(constraint,
			...
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
				if (specificationSettings.getValue() == "") {
					EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
							editingContext, this, specificationSettings, editingContext.getAdapterFactory());
					...
					EObjectPropertiesEditionContext context = new EObjectPropertiesEditionContext(
							editingContext, this, (EObject)specificationSettings.getValue(),
							editingContext.getAdapterFactory());
					PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext
							.getAdapterFactory()
							.adapt(specificationSettings.getValue(), PropertiesEditingProvider.class);
					if (provider != null) {
						PropertiesEditingPolicy policy = provider.getPolicy(context);
						if (policy != null) {
							policy.execute();
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getConstraint_Specification().equals(msg.getFeature())
					&& generalPart != null && isAccessible(UmlViewsRepository.General.specification))
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				|| key == UmlViewsRepository.General.specification;
	}
