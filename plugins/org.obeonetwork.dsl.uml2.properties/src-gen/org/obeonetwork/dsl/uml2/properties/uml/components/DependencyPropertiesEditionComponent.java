...
public class DependencyPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public DependencyPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject dependency,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, dependency.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(dependency,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						dependency.getVisibility());
			}
			if (isAccessible(UmlViewsRepository.General.supplier)) {
				supplierSettings = new ReferencesTableSettings(dependency,
			...
			if (isAccessible(UmlViewsRepository.General.client)) {
				clientSettings = new ReferencesTableSettings(dependency,
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			dependency.setName((java.lang.String)EEFConverterUtil
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getDependency_Supplier().equals(msg.getFeature())
					&& isAccessible(UmlViewsRepository.General.supplier))
				generalPart.updateSupplier();
			if (UMLPackage.eINSTANCE.getDependency_Client().equals(msg.getFeature())
					&& isAccessible(UmlViewsRepository.General.client))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getNamedElement_Visibility(),
				UMLPackage.eINSTANCE.getDependency_Supplier(), UMLPackage.eINSTANCE.getDependency_Client());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				|| key == UmlViewsRepository.General.supplier || key == UmlViewsRepository.General.client;
	}
