	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, actor.getName()));
			
			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(EEFUtils.choiceOfValues(actor, UMLPackage.eINSTANCE.getNamedElement_Visibility()), actor.getVisibility());
			}
			generalPart.setAbstract_(actor.isAbstract());
			
			generalPart.setLeaf(actor.isLeaf());
