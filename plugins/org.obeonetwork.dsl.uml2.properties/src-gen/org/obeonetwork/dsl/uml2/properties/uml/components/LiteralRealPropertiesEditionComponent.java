...
public class LiteralRealPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public LiteralRealPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject literalReal,
			String editing_mode) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.integerValue)) {
				generalPart.setIntegerValue(
						EEFConverterUtil.convertToString(TypesPackage.Literals.REAL, literalReal.getValue()));
			}
			...
			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(literalReal,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						literalReal.getVisibility());
			}
			// init filters
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.integerValue == event.getAffectedEditor()) {
			literalReal.setValue((EEFConverterUtil.createDoubleFromString(TypesPackage.Literals.REAL,
					(String)event.getNewValue())));
		}
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			literalReal.setName((java.lang.String)EEFConverterUtil
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getLiteralReal_Value().equals(msg.getFeature())
					&& msg.getNotifier().equals(semanticObject) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.integerValue)) {
				if (msg.getNewValue() != null) {
					generalPart.setIntegerValue(
							EcoreUtil.convertToString(TypesPackage.Literals.REAL, msg.getNewValue()));
				} else {
					generalPart.setIntegerValue("");
				}
			}
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				UMLPackage.eINSTANCE.getLiteralReal_Value(), UMLPackage.eINSTANCE.getNamedElement_Name(),
				UMLPackage.eINSTANCE.getNamedElement_Visibility());
				...
				if (UmlViewsRepository.General.integerValue == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(
								UMLPackage.eINSTANCE.getLiteralReal_Value().getEAttributeType(),
								(String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(
							UMLPackage.eINSTANCE.getLiteralReal_Value().getEAttributeType(), newValue);
				}
				if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
