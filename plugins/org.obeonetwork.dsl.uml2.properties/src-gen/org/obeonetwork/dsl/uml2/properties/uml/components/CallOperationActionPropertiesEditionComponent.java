...
public class CallOperationActionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public CallOperationActionPropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject callOperationAction, String editing_mode) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING,
						callOperationAction.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(callOperationAction,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						callOperationAction.getVisibility());
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			callOperationAction.setName((java.lang.String)EEFConverterUtil
