...
public class StatePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public StatePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject state,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, state.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(state, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						state.getVisibility());
			}
			...
			if (isAccessible(UmlViewsRepository.General.submachine)) {
				// init part
				submachineSettings = new EObjectFlatComboSettings(state,
						UMLPackage.eINSTANCE.getState_Submachine());
				submachineCreateSettings = new ReferencesTableSettings(getsubmachineCreateSettingsSource(),
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			state.setName((java.lang.String)EEFConverterUtil.createFromString(TypesPackage.Literals.STRING,
		...
		if (UmlViewsRepository.General.entry == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
						editingContext, this, entrySettings, editingContext.getAdapterFactory());
		...
		if (UmlViewsRepository.General.exit == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
						editingContext, this, exitSettings, editingContext.getAdapterFactory());
				PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext
						.getAdapterFactory().adapt(semanticObject, PropertiesEditingProvider.class);
				if (provider != null) {
					PropertiesEditingPolicy policy = provider.getPolicy(context);
		...
		if (UmlViewsRepository.General.do_ == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
						editingContext, this, do_Settings, editingContext.getAdapterFactory());
				PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext
						.getAdapterFactory().adapt(semanticObject, PropertiesEditingProvider.class);
				if (provider != null) {
					PropertiesEditingPolicy policy = provider.getPolicy(context);
		...
		if (UmlViewsRepository.General.submachine == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
						editingContext, this, submachineCreateSettings, editingContext.getAdapterFactory());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getState_Entry().equals(msg.getFeature()) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.entry))
				generalPart.setEntry((EObject)msg.getNewValue());
			if (UMLPackage.eINSTANCE.getState_Exit().equals(msg.getFeature()) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.exit))
				generalPart.setExit((EObject)msg.getNewValue());
			if (UMLPackage.eINSTANCE.getState_DoActivity().equals(msg.getFeature()) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.do_))
				generalPart.setDo_((EObject)msg.getNewValue());
			if (UMLPackage.eINSTANCE.getState_Submachine().equals(msg.getFeature()) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.submachine))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getNamedElement_Visibility(), UMLPackage.eINSTANCE.getState_Entry(),
				UMLPackage.eINSTANCE.getState_Exit(), UMLPackage.eINSTANCE.getState_DoActivity(),
				UMLPackage.eINSTANCE.getState_Submachine());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				|| key == UmlViewsRepository.General.entry || key == UmlViewsRepository.General.exit
