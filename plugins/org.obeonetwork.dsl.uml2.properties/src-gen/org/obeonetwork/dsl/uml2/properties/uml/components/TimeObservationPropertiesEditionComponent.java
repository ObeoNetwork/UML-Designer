...
public class TimeObservationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (timeObservation.getName() != null && isAccessible(UmlViewsRepository.TimeObservation.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(UMLPackage.eINSTANCE.getString(), timeObservation.getName()));
			
			if (isAccessible(UmlViewsRepository.TimeObservation.Properties.visibility)) {
				basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), timeObservation.getVisibility());
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.TimeObservation.Properties.name == event.getAffectedEditor()) {
			timeObservation.setName((java.lang.String)EEFConverterUtil.createFromString(UMLPackage.eINSTANCE.getString(), (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.TimeObservation.Properties.name)) {
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Visibility().equals(msg.getFeature()) && isAccessible(UmlViewsRepository.TimeObservation.Properties.visibility))
			...
			if (UMLPackage.eINSTANCE.getTimeObservation_FirstEvent().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.TimeObservation.Properties.firstEvent))
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.TimeObservation.Properties.firstEvent == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getTimeObservation_FirstEvent().getEAttributeType(), (String)newValue);
					}
