...
public class CallEventPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public CallEventPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject callEvent,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, callEvent.getName()));

			if (isAccessible(UmlViewsRepository.General.operation)) {
				// init part
				operationSettings = new EObjectFlatComboSettings(callEvent,
						UMLPackage.eINSTANCE.getCallEvent_Operation());
				generalPart.initOperation(operationSettings);
				// set the button mode
				generalPart.setOperationButtonMode(ButtonsModeEnum.BROWSE);
			}
			// init filters

			if (isAccessible(UmlViewsRepository.General.operation)) {
				generalPart.addFilterToOperation(new ViewerFilter() {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == UmlViewsRepository.General.operation) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			callEvent.setName((java.lang.String)EEFConverterUtil
					.createFromString(TypesPackage.Literals.STRING, (String)event.getNewValue()));
		}
		if (UmlViewsRepository.General.operation == event.getAffectedEditor()) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getCallEvent_Operation().equals(msg.getFeature()) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.operation))
				generalPart.setOperation((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				UMLPackage.eINSTANCE.getNamedElement_Name(), UMLPackage.eINSTANCE.getCallEvent_Operation());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.operation;
	}
	...
	public boolean isRequired(Object key, int kind) {
		return key == UmlViewsRepository.General.operation;
	}
