...
public class DurationObservationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public DurationObservationPropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject durationObservation, String editing_mode) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING,
						durationObservation.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(durationObservation,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						durationObservation.getVisibility());
			}
			...
			// firstEvent, General, DurationObservation.
			// init filters
			...
			// module, with the values : firstEvent, General, DurationObservation.
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			durationObservation.setName((java.lang.String)EEFConverterUtil
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				|| key == UmlViewsRepository.General.booleanValue;
	}
				...
				if (UmlViewsRepository.General.booleanValue == event.getAffectedEditor()) {
					...
					for (Iterator iterator = ((List)event.getNewValue()).iterator(); iterator.hasNext();) {
						chain.add(Diagnostician.INSTANCE.validate(
								UMLPackage.eINSTANCE.getDurationObservation_FirstEvent().getEAttributeType(),
								iterator.next()));
					}
					ret = chain;
				}
