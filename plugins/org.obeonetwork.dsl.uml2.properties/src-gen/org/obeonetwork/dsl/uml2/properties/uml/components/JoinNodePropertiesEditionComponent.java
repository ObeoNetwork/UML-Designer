...
public class JoinNodePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (joinNode.getName() != null && isAccessible(UmlViewsRepository.JoinNode.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(UMLPackage.eINSTANCE.getString(), joinNode.getName()));
			
			if (isAccessible(UmlViewsRepository.JoinNode.Properties.visibility)) {
				basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), joinNode.getVisibility());
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.JoinNode.Properties.name == event.getAffectedEditor()) {
			joinNode.setName((java.lang.String)EEFConverterUtil.createFromString(UMLPackage.eINSTANCE.getString(), (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.JoinNode.Properties.name)) {
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Visibility().equals(msg.getFeature()) && isAccessible(UmlViewsRepository.JoinNode.Properties.visibility))
			...
			if (UMLPackage.eINSTANCE.getRedefinableElement_IsLeaf().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.JoinNode.Properties.isLeaf))
			...
			if (UMLPackage.eINSTANCE.getJoinNode_IsCombineDuplicate().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.JoinNode.Properties.isCombineDuplicate))
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.JoinNode.Properties.isCombineDuplicate == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getJoinNode_IsCombineDuplicate().getEAttributeType(), (String)newValue);
					}
