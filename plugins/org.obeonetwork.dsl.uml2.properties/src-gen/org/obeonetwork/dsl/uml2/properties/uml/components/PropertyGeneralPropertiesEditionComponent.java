...
public class PropertyGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public PropertyGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext,
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, property.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(property, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
			...
			if (isAccessible(UmlViewsRepository.General.aggregation)) {
				generalPart.initAggregation(
						EEFUtils.choiceOfValues(property, UMLPackage.eINSTANCE.getProperty_Aggregation()),
						property.getAggregation());
			}
			if (isAccessible(UmlViewsRepository.General.lowerValue)) {
				generalPart.setLowerValue(
						EEFConverterUtil.convertToString(TypesPackage.Literals.INTEGER, property.getLower()));
			}

			if (isAccessible(UmlViewsRepository.General.upperValue)) {
				generalPart.setUpperValue(EEFConverterUtil
						.convertToString(TypesPackage.Literals.UNLIMITED_NATURAL, property.getUpper()));
			}

			if (isAccessible(UmlViewsRepository.General.defaultValue)) {
				// init part
				defaultValueSettings = new EObjectFlatComboSettings(property,
			...
			if (isAccessible(UmlViewsRepository.General.type)) {
				// init part
				typeSettings = new EObjectFlatComboSettings(property,
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			property.setName((java.lang.String)EEFConverterUtil.createFromString(TypesPackage.Literals.STRING,
		...
		if (UmlViewsRepository.General.lowerValue == event.getAffectedEditor()) {
			property.setLower((EEFConverterUtil.createIntFromString(TypesPackage.Literals.INTEGER,
					(String)event.getNewValue())));
		}
		if (UmlViewsRepository.General.upperValue == event.getAffectedEditor()) {
			property.setUpper((EEFConverterUtil.createIntFromString(TypesPackage.Literals.UNLIMITED_NATURAL,
					(String)event.getNewValue())));
		}
		if (UmlViewsRepository.General.defaultValue == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				...
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(
						editingContext, this, defaultValueSettings, editingContext.getAdapterFactory());
				PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getProperty_DefaultValue().equals(msg.getFeature())
					&& generalPart != null && isAccessible(UmlViewsRepository.General.defaultValue))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getProperty_DefaultValue(), UMLPackage.eINSTANCE.getTypedElement_Type());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				...
				|| key == UmlViewsRepository.General.defaultValue || key == UmlViewsRepository.General.type
