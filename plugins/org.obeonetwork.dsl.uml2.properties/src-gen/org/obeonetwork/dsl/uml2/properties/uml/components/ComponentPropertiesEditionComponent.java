...
public class ComponentPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public ComponentPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject component,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, component.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(component, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
			...
			if (isAccessible(UmlViewsRepository.General.usecase)) {
				usecaseSettings = new ReferencesTableSettings(component,
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			component.setName((java.lang.String)EEFConverterUtil
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getComponent_IsIndirectlyInstantiated().equals(msg.getFeature())
					&& msg.getNotifier().equals(semanticObject) && generalPart != null
					&& isAccessible(UmlViewsRepository.General.Qualifiers.indirectlyInstantiated))
			...
			if (UMLPackage.eINSTANCE.getClassifier_UseCase().equals(msg.getFeature())
					&& isAccessible(UmlViewsRepository.General.usecase))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getRedefinableElement_IsLeaf(), UMLPackage.eINSTANCE.getClass_IsActive(),
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				...
				|| key == UmlViewsRepository.General.Qualifiers.indirectlyInstantiated
				|| key == UmlViewsRepository.General.usecase
	...
	public Diagnostic validateValue(IPropertiesEditionEvent event) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.General.Qualifiers.indirectlyInstantiated == event
						.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(UMLPackage.eINSTANCE
								.getComponent_IsIndirectlyInstantiated().getEAttributeType(),
								(String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(
							UMLPackage.eINSTANCE.getComponent_IsIndirectlyInstantiated().getEAttributeType(),
							newValue);
				}
