...
public class ConnectorGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public ConnectorGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject connector, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, connector.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(connector, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						connector.getVisibility());
			}
			generalPart.setLeaf(connector.isLeaf());

			if (isAccessible(UmlViewsRepository.General.kind_readonly)) {
				generalPart.initKind_readonly(
						EEFUtils.choiceOfValues(connector, UMLPackage.eINSTANCE.getConnector_Kind()),
						connector.getKind());
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			connector.setName((java.lang.String)EEFConverterUtil
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getConnector_Kind().equals(msg.getFeature())
					&& msg.getNotifier().equals(semanticObject)
					&& isAccessible(UmlViewsRepository.General.kind_readonly))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				...
				UMLPackage.eINSTANCE.getRedefinableElement_IsLeaf(), UMLPackage.eINSTANCE.getConnector_Kind(),
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				...
				|| key == UmlViewsRepository.General.kind_readonly
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.General.kind_readonly == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(
								UMLPackage.eINSTANCE.getConnector_Kind().getEAttributeType(),
								(String)newValue);
					}
					ret = Diagnostician.INSTANCE
							.validate(UMLPackage.eINSTANCE.getConnector_Kind().getEAttributeType(), newValue);
				}
				if (UmlViewsRepository.General.Qualifiers.static_ == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
