 ...
 * @generated
 */
public class AssociationPropertiesEditionComponent extends
	 ...
	 * @generated
	 */
	public AssociationPropertiesEditionComponent(
	...
	public void initPart(Object key, int kind, EObject elt,
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(EEFConverterUtil.convertToString(
						TypesPackage.Literals.STRING, association.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(EEFUtils.choiceOfValues(association,
						UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						association.getVisibility());
			}
			...
			if (isAccessible(UmlViewsRepository.General.memberEnd)) {
				memberEndSettings = new ReferencesTableSettings(association,
			...
			if (isAccessible(UmlViewsRepository.General.memberEnd)) {
				generalPart.addFilterToMemberEnd(new EObjectFilter(
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			association.setName((java.lang.String) EEFConverterUtil
		...
		if (UmlViewsRepository.General.Qualifiers.abstract_ == event
				.getAffectedEditor()) {
			association.setIsAbstract((Boolean) event.getNewValue());
		}
		if (UmlViewsRepository.General.Qualifiers.leaf == event
				.getAffectedEditor()) {
			association.setIsLeaf((Boolean) event.getNewValue());
		}
		if (UmlViewsRepository.General.Qualifiers.derived == event
				.getAffectedEditor()) {
		...
		if (UmlViewsRepository.General.memberEnd == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof Property) {
					memberEndSettings.addToReference((EObject) event
							.getNewValue());
				}
			} else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
				memberEndSettings.removeFromReference((EObject) event
						.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.MOVE) {
				memberEndSettings.move(event.getNewIndex(),
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getRedefinableElement_IsLeaf().equals(
					msg.getFeature())
					&& msg.getNotifier().equals(semanticObject)
					&& generalPart != null
			...
			if (UMLPackage.eINSTANCE.getProperty_IsDerived().equals(
					msg.getFeature())
					&& msg.getNotifier().equals(semanticObject)
					&& generalPart != null
			...
			if (UMLPackage.eINSTANCE.getAssociation_MemberEnd().equals(
					msg.getFeature())
	...
	public Diagnostic validateValue(IPropertiesEditionEvent event) {
		...
		if (event.getNewValue() != null) {
			try {
				if (UmlViewsRepository.General.name == event
						.getAffectedEditor()) {
					...
					ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE
							.getNamedElement_Name().getEAttributeType(),
							newValue);
				}
				if (UmlViewsRepository.General.visibility == event
						.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil
					...
					ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE
							.getNamedElement_Visibility().getEAttributeType(),
							newValue);
				}
				if (UmlViewsRepository.General.Qualifiers.abstract_ == event
						.getAffectedEditor()) {
					...
					ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE
							.getClassifier_IsAbstract().getEAttributeType(),
							newValue);
				}
				if (UmlViewsRepository.General.Qualifiers.leaf == event
						.getAffectedEditor()) {
				...
				if (UmlViewsRepository.General.Qualifiers.derived == event
					...
					ret = Diagnostician.INSTANCE.validate(UMLPackage.eINSTANCE
							.getProperty_IsDerived().getEAttributeType(),
							newValue);
				}
