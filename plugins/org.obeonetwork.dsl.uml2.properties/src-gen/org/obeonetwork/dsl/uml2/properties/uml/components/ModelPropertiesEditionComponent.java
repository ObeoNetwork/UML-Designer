...
public class ModelPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (model.getName() != null && isAccessible(UmlViewsRepository.Model.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(UMLPackage.eINSTANCE.getString(), model.getName()));
			
			if (isAccessible(UmlViewsRepository.Model.Properties.visibility)) {
				basePart.initVisibility((EEnum) UMLPackage.eINSTANCE.getNamedElement_Visibility().getEType(), model.getVisibility());
			}
			...
			if (model.getViewpoint() != null && isAccessible(UmlViewsRepository.Model.Properties.viewpoint))
				basePart.setViewpoint(EEFConverterUtil.convertToString(UMLPackage.eINSTANCE.getString(), model.getViewpoint()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.Model.Properties.name == event.getAffectedEditor()) {
			model.setName((java.lang.String)EEFConverterUtil.createFromString(UMLPackage.eINSTANCE.getString(), (String)event.getNewValue()));
		}
		...
		if (UmlViewsRepository.Model.Properties.viewpoint == event.getAffectedEditor()) {
			model.setViewpoint((java.lang.String)EEFConverterUtil.createFromString(UMLPackage.eINSTANCE.getString(), (String)event.getNewValue()));
		}
	}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.Model.Properties.name)) {
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Visibility().equals(msg.getFeature()) && isAccessible(UmlViewsRepository.Model.Properties.visibility))
			...
			if (UMLPackage.eINSTANCE.getModel_Viewpoint().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.Model.Properties.viewpoint)) {
				if (msg.getNewValue() != null) {
					basePart.setViewpoint(EcoreUtil.convertToString(UMLPackage.eINSTANCE.getString(), msg.getNewValue()));
				} else {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.Model.Properties.viewpoint == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getModel_Viewpoint().getEAttributeType(), (String)newValue);
					}
