...
public class LiteralIntegerPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public LiteralIntegerPropertiesEditionComponent(PropertiesEditingContext editingContext,
			EObject literalInteger, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.integerValue)) {
				generalPart.setIntegerValue(EEFConverterUtil.convertToString(TypesPackage.Literals.INTEGER,
						literalInteger.getValue()));
			}
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING,
						literalInteger.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(literalInteger,
								UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						literalInteger.getVisibility());
			}
			// init filters
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.integerValue == event.getAffectedEditor()) {
			literalInteger.setValue((EEFConverterUtil.createIntFromString(TypesPackage.Literals.INTEGER,
					(String)event.getNewValue())));
		}
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			literalInteger.setName((java.lang.String)EEFConverterUtil
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (UMLPackage.eINSTANCE.getLiteralInteger_Value().equals(msg.getFeature())
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
				UMLPackage.eINSTANCE.getLiteralInteger_Value(), UMLPackage.eINSTANCE.getNamedElement_Name(),
				UMLPackage.eINSTANCE.getNamedElement_Visibility());
				...
				if (UmlViewsRepository.General.integerValue == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(
								UMLPackage.eINSTANCE.getLiteralInteger_Value().getEAttributeType(),
								(String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(
							UMLPackage.eINSTANCE.getLiteralInteger_Value().getEAttributeType(), newValue);
				}
				if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
