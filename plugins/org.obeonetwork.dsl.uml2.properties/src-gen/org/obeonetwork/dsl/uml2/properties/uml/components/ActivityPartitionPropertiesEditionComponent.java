...
public class ActivityPartitionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (activityPartition.getName() != null && isAccessible(UmlViewsRepository.ActivityPartition.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(UMLPackage.eINSTANCE.getString(), activityPartition.getName()));
			...
			// Start of user code for additional businessfilters for superPartition
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.ActivityPartition.Properties.name == event.getAffectedEditor()) {
			activityPartition.setName((java.lang.String)EEFConverterUtil.createFromString(UMLPackage.eINSTANCE.getString(), (String)event.getNewValue()));
		}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.ActivityPartition.Properties.name)) {
			...
			if (UMLPackage.eINSTANCE.getNamedElement_Visibility().equals(msg.getFeature()) && isAccessible(UmlViewsRepository.ActivityPartition.Properties.visibility))
			...
			if (UMLPackage.eINSTANCE.getActivityPartition_IsDimension().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.ActivityPartition.Properties.isDimension))
				basePart.setIsDimension((Boolean)msg.getNewValue());
			
			if (UMLPackage.eINSTANCE.getActivityPartition_IsExternal().equals(msg.getFeature()) && basePart != null && isAccessible(UmlViewsRepository.ActivityPartition.Properties.isExternal))
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (UmlViewsRepository.ActivityPartition.Properties.isDimension == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getActivityPartition_IsDimension().getEAttributeType(), (String)newValue);
					}
				...
				if (UmlViewsRepository.ActivityPartition.Properties.isExternal == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(UMLPackage.eINSTANCE.getActivityPartition_IsExternal().getEAttributeType(), (String)newValue);
					}
