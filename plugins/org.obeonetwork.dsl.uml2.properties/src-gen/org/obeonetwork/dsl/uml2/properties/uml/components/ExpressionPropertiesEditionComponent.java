...
public class ExpressionPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(UmlViewsRepository.General.name))
                generalPart.setName(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, expression.getName()));

            if (isAccessible(UmlViewsRepository.General.visibility)) {
                generalPart.initVisibility(EEFUtils.choiceOfValues(expression, UMLPackage.eINSTANCE.getNamedElement_Visibility()), expression.getVisibility());
            }
            if (isAccessible(UmlViewsRepository.General.symbol))
                generalPart.setSymbol(EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, expression.getSymbol()));

            if (isAccessible(UmlViewsRepository.General.operand)) {
                operandSettings = new ReferencesTableSettings(expression, UMLPackage.eINSTANCE.getExpression_Operand());
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
            expression.setName((java.lang.String) EEFConverterUtil.createFromString(TypesPackage.Literals.STRING, (String) event.getNewValue()));
        }
        ...
        if (UmlViewsRepository.General.symbol == event.getAffectedEditor()) {
            expression.setSymbol((java.lang.String) EEFConverterUtil.createFromString(TypesPackage.Literals.STRING, (String) event.getNewValue()));
        }
        if (UmlViewsRepository.General.operand == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.ADD) {
                EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext, this, operandSettings, editingContext.getAdapterFactory());
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (UMLPackage.eINSTANCE.getExpression_Symbol().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null
                    && isAccessible(UmlViewsRepository.General.symbol)) {
                if (msg.getNewValue() != null) {
                    generalPart.setSymbol(EcoreUtil.convertToString(TypesPackage.Literals.STRING, msg.getNewValue()));
                } else {
                    generalPart.setSymbol("");
                }
            }
            if (UMLPackage.eINSTANCE.getExpression_Operand().equals(msg.getFeature()) && isAccessible(UmlViewsRepository.General.operand))
    ...
    public boolean mustBeComposed(Object key, int kind) {
        return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility || key == UmlViewsRepository.General.symbol || key == UmlViewsRepository.General.operand;
    }
