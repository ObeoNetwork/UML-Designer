...
public class PinPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * @generated
	 */
	public PinPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject pin,
			String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(UmlViewsRepository.General.name))
				generalPart.setName(
						EEFConverterUtil.convertToString(TypesPackage.Literals.STRING, pin.getName()));

			if (isAccessible(UmlViewsRepository.General.visibility)) {
				generalPart.initVisibility(
						EEFUtils.choiceOfValues(pin, UMLPackage.eINSTANCE.getNamedElement_Visibility()),
						pin.getVisibility());
			}
			...
			if (isAccessible(UmlViewsRepository.General.lowerValue)) {
				generalPart.setLowerValue(
						EEFConverterUtil.convertToString(TypesPackage.Literals.INTEGER, pin.getLower()));
			}

			if (isAccessible(UmlViewsRepository.General.upperValue)) {
				generalPart.setUpperValue(EEFConverterUtil
						.convertToString(TypesPackage.Literals.UNLIMITED_NATURAL, pin.getUpper()));
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (UmlViewsRepository.General.name == event.getAffectedEditor()) {
			pin.setName((java.lang.String)EEFConverterUtil.createFromString(TypesPackage.Literals.STRING,
		...
		if (UmlViewsRepository.General.lowerValue == event.getAffectedEditor()) {
			pin.setLower((EEFConverterUtil.createIntFromString(TypesPackage.Literals.INTEGER,
					(String)event.getNewValue())));
		}
		if (UmlViewsRepository.General.upperValue == event.getAffectedEditor()) {
			pin.setUpper((EEFConverterUtil.createIntFromString(TypesPackage.Literals.UNLIMITED_NATURAL,
					(String)event.getNewValue())));
		}
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == UmlViewsRepository.General.name || key == UmlViewsRepository.General.visibility
				...
				|| key == UmlViewsRepository.General.upperValue || key == UmlViewsRepository.General.type
