 ...
 */
public class MemberEndsTable extends ReferencesTable implements
	 ...
	 * Set column names
	 */
	private String[] columnNames = new String[] { PROPERTY_COLUMN,
			NAVIGABLE_COLUMN, OWNED_COLUMN };
	 ...
	 */
	public MemberEndsTable(String labeltoDisplay,
			MemberEndsTableListener referenceListener) {
	...
	private void computeAddButtonStatus() {
		if (getTable().getEnabled()
				&& (this.upperBound < 0 || getSize() < this.upperBound))
		...
		public void mouseUp(MouseEvent e) {
			...
			for (int i = (tableItems.length - 1); i >= 0; i--) {
				...
				if (newIndex >= 0 && newIndex < getSize()) {
					// Move
					referencesTableListener.handleMove(
							(EObject) tableItems[i].getData(), newIndex + 1,
							newIndex);
				}
			}
