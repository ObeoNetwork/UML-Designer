 ...
 */
public class NamedElementStereotypesPropertiesEditionComponent extends SinglePartPropertiesEditingComponent implements ITableOperations {
     ...
     * Default constructor
     */
    public NamedElementStereotypesPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject class_, String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(CustomUmlViewsRepository.Stereotypes.appliedStereotypes)) {
                appliedStereotypesSettings = new OperationsTableSettings(semanticObject, this);
                stereotypesPart.initAppliedStereotypes((OperationsTableSettings) appliedStereotypesSettings);
            }
            // init filters
            if (isAccessible(CustomUmlViewsRepository.Stereotypes.appliedStereotypes)) {
                stereotypesPart.addFilterToAppliedStereotypes(new EObjectFilter(UMLPackage.Literals.STEREOTYPE));
            }
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (CustomUmlViewsRepository.Stereotypes.appliedStereotypes == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.ADD) {
                if (event.getNewValue() instanceof NamedElement) {
                    appliedStereotypesSettings.addToReference((EObject) event.getNewValue());
                }
            } else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
                ((OperationsTableSettings) appliedStereotypesSettings).removeFromReference((EObject) event.getNewValue());
    ...
    public Object[] getValues() {
        List<Stereotype> list = ((NamedElement) semanticObject).getAppliedStereotypes();
    ...
    public void remove(EObject valueToRemove) {
        ((NamedElement) semanticObject).unapplyStereotype((Stereotype) valueToRemove);
    ...
    public Object choiceOfValues() {
        ...
        List<Stereotype> appliedStereotypes = namedElement.getAppliedStereotypes();
