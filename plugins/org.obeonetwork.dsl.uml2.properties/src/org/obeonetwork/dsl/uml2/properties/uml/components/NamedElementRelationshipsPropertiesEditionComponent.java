 ...
 */
public class NamedElementRelationshipsPropertiesEditionComponent extends
	 ...
	 * Default constructor
	 */
	public NamedElementRelationshipsPropertiesEditionComponent(
	...
	public void initPart(Object key, int kind, EObject elt,
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(CustomUmlViewsRepository.Relationships.relationshipsOriginating)) {
				relationshipsOriginatingSettings = new OperationsTableSettings(
						semanticObject,
						new RelationshipsOriginatingTableSettings(
								semanticObject));
				relationshipsPart
						.initRelationshipsOriginating((OperationsTableSettings) relationshipsOriginatingSettings);
			}
			if (isAccessible(CustomUmlViewsRepository.Relationships.relationshipsTargeting)) {
				relationshipsTargetingSettings = new OperationsTableSettings(
						semanticObject,
						new RelationshipsTargetingTableSettings(semanticObject));
				relationshipsPart
						.initRelationshipsTargeting((OperationsTableSettings) relationshipsTargetingSettings);
			}

			// init filters
			if (isAccessible(CustomUmlViewsRepository.Relationships.relationshipsOriginating)) {
				relationshipsPart
						.addFilterToRelationshipsOriginating(new EObjectFilter(
								UMLPackage.Literals.RELATIONSHIP));
			}
			if (isAccessible(CustomUmlViewsRepository.Relationships.relationshipsTargeting)) {
				relationshipsPart
						.addFilterToRelationshipsTargeting(new EObjectFilter(
								UMLPackage.Literals.RELATIONSHIP));
			}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (CustomUmlViewsRepository.Relationships.relationshipsOriginating == event
				.getAffectedEditor()) {
		...
		if (CustomUmlViewsRepository.Relationships.relationshipsTargeting == event
				.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
