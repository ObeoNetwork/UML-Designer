 ...
 */
public class ElementDocumentationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
    ...
    protected EEFEditorSettingsImpl commentSettings = (EEFEditorSettingsImpl) EEFEditorSettingsBuilder.create(semanticObject, UMLPackage.eINSTANCE.getComment_Body())
            .nextStep(NavigationStepBuilder.create(UMLPackage.eINSTANCE.getElement_OwnedComment()).index(0).init(new EEFInitializer() {
     ...
     * Default constructor
     */
    public ElementDocumentationPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject class_, String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            // init values
            if (commentSettings.getValue() != null && isAccessible(CustomUmlViewsRepository.Documentation.comment))
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        if (CustomUmlViewsRepository.Documentation.comment == event.getAffectedEditor()) {
            Object oldValue = commentSettings.getValue();
            Object newValue = event.getNewValue();
            // Discard event if value does not change
            if (oldValue == null && "".equals(newValue) || newValue.equals(oldValue))
                return;
            commentSettings.setValue((java.lang.String) EEFConverterUtil.createFromString(TypesPackage.Literals.STRING, (String) newValue));
        }
    }
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (!(msg.getNewValue() instanceof EObject) && commentSettings.isAffectingEvent(msg) && documentationPart != null && isAccessible(CustomUmlViewsRepository.Documentation.comment)) {
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(UMLPackage.eINSTANCE.getComment_Body(), UMLPackage.eINSTANCE.getElement_OwnedComment());
