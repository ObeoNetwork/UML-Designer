...
public class DocumentationPropertiesEditionPartForm extends
	...
	public void createControls(final FormToolkit widgetFactory, Composite view) {
		CompositionSequence documentationStep = new BindingCompositionSequence(
				propertiesEditionComponent);
		documentationStep
				.addStep(CustomUmlViewsRepository.Documentation.comment);
	...
	protected Composite createCommentTextarea(FormToolkit widgetFactory,
			Composite parent) {
		Label commentLabel = createDescription(
				parent,
				CustomUmlViewsRepository.Documentation.comment,
				CustomUmlMessages.DocumentationPropertiesEditionPart_CommentLabel);
		...
		comment = widgetFactory.createText(parent,
				"", SWT.BORDER | SWT.WRAP | SWT.MULTI | SWT.V_SCROLL); //$NON-NLS-1$
				...
				if (propertiesEditionComponent != null) {
					propertiesEditionComponent
							.firePropertiesChanged(new PropertiesEditionEvent(
									...
									PropertiesEditionEvent.SET, null, comment
											.getText()));
					propertiesEditionComponent
							.firePropertiesChanged(new PropertiesEditionEvent(
									DocumentationPropertiesEditionPartForm.this,
									...
									PropertiesEditionEvent.FOCUS_LOST, null,
									comment.getText()));
				}
			}
				...
				if (propertiesEditionComponent != null) {
					propertiesEditionComponent
							.firePropertiesChanged(new PropertiesEditionEvent(
									...
									PropertiesEditionEvent.FOCUS_GAINED, null,
									null));
				}
			}
		});
		EditingUtils.setID(comment,
				CustomUmlViewsRepository.Documentation.comment);
	...
	public void setComment(String newValue) {
		...
		boolean eefElementEditorReadOnlyState = isReadOnly(
				CustomUmlViewsRepository.Documentation.comment,
				((ElementDocumentationPropertiesEditionComponent) propertiesEditionComponent)
						.getCommentSettings().getOrCreateSignificantObject());
		if (eefElementEditorReadOnlyState && comment.isEnabled()) {
			comment.setEnabled(false);
			comment.setBackground(comment.getDisplay().getSystemColor(
