 ...
 */
public class PackageProfilesPropertiesEditionComponent extends SinglePartPropertiesEditingComponent implements ITableOperations {
     ...
     * Default constructor
     */
    public PackageProfilesPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject class_, String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(CustomUmlViewsRepository.Profiles.appliedProfiles)) {
                appliedProfilesSettings = new OperationsTableSettings(semanticObject, this);
                profilesPart.initAppliedProfiles((OperationsTableSettings) appliedProfilesSettings);
            }
            // init filters
            if (isAccessible(CustomUmlViewsRepository.Profiles.appliedProfiles)) {
                profilesPart.addFilterToAppliedProfiles(new EObjectFilter(UMLPackage.Literals.PROFILE));
            }
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (CustomUmlViewsRepository.Profiles.appliedProfiles == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.ADD) {
                if (event.getNewValue() instanceof NamedElement) {
                    appliedProfilesSettings.addToReference((EObject) event.getNewValue());
                }
            } else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
                ((OperationsTableSettings) appliedProfilesSettings).removeFromReference((EObject) event.getNewValue());
    ...
    public Object choiceOfValues() {
        ...
        if (session != null && session.getSemanticResources() != null) {
            for (Resource resource : session.getSemanticResources()) {
                applicableProfiles.addAll(Lists.newArrayList(Iterators.filter(resource.getContents().iterator(), Profile.class)));
            }
        }
