 ...
 *******************************************************************************/
package org.obeonetwork.dsl.uml2.core.internal.commands;
...
import org.obeonetwork.dsl.uml2.core.internal.services.CreateElementLabelFromModelExplorerSwitch;
...
public abstract class AbstractCreateDiagram<T extends PackageableElement> extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if (session != null) {
            ...
            final RecordingCommand cmd = new RecordingCommand(session.getTransactionalEditingDomain(), "Create Diagram") { //$NON-NLS-1$
                @Override
                protected void doExecute() {
                    for (final Package pkg : packages) {
                        ...
                        final RepresentationDescription diagDescription = getDiagramDescription(session, semanticElement);

                        final String representationName = getRepresentationName(diagDescription, getDiagramLabel(semanticElement));
                        if (representationName != null) {
                            // Create representation
                            final DRepresentation representation = DialectManager.INSTANCE.createRepresentation(representationName, semanticElement, diagDescription, session,
