...
/**
 * Listen to Create new child command in model explorer in order to set a
 * default name to the newly created element and select it in the treeviwer.
 *
...
public class CreateNewChildListener implements CommandStackListener {
        ...
        if (mostRecentCommand != null && mostRecentCommand instanceof CreateChildCommand) {
            ...
            while (iterator.hasNext()) {
                ...
                if (object instanceof NamedElement && Strings.isNullOrEmpty(((NamedElement) object).getName())) {
                    ...
                    final String defaultName = LabelServices.INSTANCE.computeDefaultName((NamedElement) object);
                    // set default name
            ...
            try {
                modelExplorerView = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView("org.eclipse.sirius.ui.tools.views.model.explorer"); //$NON-NLS-1$
                ...
                UMLDesignerCorePlugin.log(IStatus.ERROR, NLS.bind(Messages.UmlModelExplorer_UI_ErrorMsg_NotFound, modelExplorerView), e);
            }
        }
    }
    ...
    private void setName(NamedElement element, String newName) {
        final EditingDomain editingDomain = SessionManager.INSTANCE.getSession(element).getTransactionalEditingDomain();
