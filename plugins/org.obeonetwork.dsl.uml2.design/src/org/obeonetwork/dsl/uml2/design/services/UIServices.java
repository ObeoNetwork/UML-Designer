...
public class UIServices {
	...
	private void drop(final Element newContainer,
		...
		if (moveSemanticElement && oldContainer != newContainer) {
			...
			TransactionalEditingDomain domain = session
					.getTransactionalEditingDomain();
			// The feature is set to null because the domain will deduce it
			Command cmd = AddCommand.create(domain, newContainer, null,
					semanticElement);
			if (cmd.canExecute()) {
				cmd.execute();
			}
			cmd = RemoveCommand.create(domain, oldContainer, null,
					semanticElement);
			if (cmd.canExecute()) {
				cmd.execute();
			}
			...
			if (semanticElement instanceof UseCase) {
				...
				cmd = SetCommand.create(domain, semanticElement,
				...
				cmd = SetCommand.create(domain, semanticElement,
						UMLPackage.Literals.USE_CASE__SUBJECT, subjects);
				if (cmd.canExecute()) {
					cmd.execute();
				}
			}
	...
	public void addExistingElements(final DSemanticDecorator containerView,
		...
		final Session session = SessionManager.INSTANCE
				.getSession(semanticElements.get(0));
