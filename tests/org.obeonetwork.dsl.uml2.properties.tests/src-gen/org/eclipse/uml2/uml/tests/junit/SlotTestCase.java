 ...
 */
public class SlotTestCase extends SWTBotEEFTestCase {
	...
	public void testEditSlotOwningInstance() throws Exception {
		...
		// Open the EEF wizard (by double click) to edit the Slot element
		EObject firstInstanceOf = EEFTestsModelsUtils.getFirstInstanceOf(bot.getActiveResource(), slotMetaClass);
		if (firstInstanceOf == null)
			throw new InputModelInvalidException(slotMetaClass.getName());

		SWTBotShell wizardShell = bot.prepareBatchEditing(modelEditor, slotMetaClass, firstInstanceOf, null);

		// Change value of the owningInstance feature of the Slot element 
		bot.editEObjectFlatComboViewerFeature(wizardShell, UmlViewsRepository.Slot.Properties.owningInstance, allInstancesOf.indexOf(referenceValueForOwningInstance));
