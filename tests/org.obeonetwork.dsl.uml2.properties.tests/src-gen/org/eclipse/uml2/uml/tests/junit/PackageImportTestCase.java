 ...
 */
public class PackageImportTestCase extends SWTBotEEFTestCase {
	...
	public void testEditPackageImportVisibility() throws Exception {
		...
		// Open the EEF wizard (by double click) to edit the PackageImport element
		EObject firstInstanceOf = EEFTestsModelsUtils.getFirstInstanceOf(bot.getActiveResource(), packageImportMetaClass);
		if (firstInstanceOf == null)
			throw new InputModelInvalidException(packageImportMetaClass.getName());

		SWTBotShell wizardShell = bot.prepareBatchEditing(modelEditor, packageImportMetaClass, firstInstanceOf, null);

		// Change value of the visibility feature of the PackageImport element 
				bot.editTextFeature(wizardShell, UmlViewsRepository.PackageImport.Properties.visibility, UPDATED_VALUE);
	...
	public void testEditPackageImportImportedPackage() throws Exception {
		...
		bot.editEObjectFlatComboViewerFeature(wizardShell, UmlViewsRepository.PackageImport.Properties.importedPackage, allInstancesOf.indexOf(referenceValueForImportedPackage));
