 ...
 */
public class CommentTestCase extends SWTBotEEFTestCase {
	...
	public void testEditCommentBody() throws Exception {
		...
		// Open the EEF wizard (by double click) to edit the Comment element
		EObject firstInstanceOf = EEFTestsModelsUtils.getFirstInstanceOf(bot.getActiveResource(), commentMetaClass);
		if (firstInstanceOf == null)
			throw new InputModelInvalidException(commentMetaClass.getName());

		SWTBotShell wizardShell = bot.prepareBatchEditing(modelEditor, commentMetaClass, firstInstanceOf, null);

		// Change value of the body feature of the Comment element 
				bot.editTextFeature(wizardShell, UmlViewsRepository.Comment.Properties.body, UPDATED_VALUE);
