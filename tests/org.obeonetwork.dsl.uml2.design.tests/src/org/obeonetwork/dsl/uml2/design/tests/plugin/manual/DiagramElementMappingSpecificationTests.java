...
public class DiagramElementMappingSpecificationTests {
	...
	private static Set<String> reconnectWhiteList = Sets.newHashSet("CD_BrokenAssociationToClasses",
			...
			"CS_U_RequiredInterface2Port", "CS_U_RequiredInterface2SubPort", "CS_C_SubProvidedInterface2Port",
			...
			"CD_StereotypeApplicationLink", "CD_BrokenAssociationToAssociation");
	...
	public void reconnect() {
		if (underTest instanceof EdgeMapping && !reconnectWhiteList.contains(underTest.getName())) {
			...
			} else if (reconnections.size() == 1 && ReconnectionKind.RECONNECT_BOTH_LITERAL != reconnections
